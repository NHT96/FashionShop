...
$('.cart-product__amount').on('change', function(){
    ...
    let totalvalue = this.value * $(this).parent().siblings('.cart-product--auto').children('.price').children('#product_price').val();
    $(this).parent().siblings('.totalprice').children('.total-value').text(totalvalue.toLocaleString('it-IT', {currency : 'VND'}).replaceAll(".",","));
})
$('.cart-product__remove').click(function(){
    var cart_id = $(this).parent().children('.cart-product__info').children('.cart-product--auto').children('#cart_id').val();
...
//
/*$.get("/public/js/json/provinces-open-api.json", function (data) {
    // Cities
